...
576
597
Average energy of sustainable agents
...
Imitation-bias
Imitation-bias
"most successful" "majority" "minority" "most experienced" "parents"
4
...
(these variables could be considered both a factor of the environment and a variable of the agents, but we list them here since they do not vary among agents and can be considered to be imposed by the environment)
...
* _Harvest-greedy_: the actual harvesting behavior of sustainable and greedy agents
* _Reproduction_: ability of agents to produce offspring
* _Imitation bias_: the kind of rule that agents follow as they imitate other neighboring agents:
...
* _Imitation likelihood_: the likelihood that agents will imitate others
* _Behavior innovation rate_: The probability with which agents switch to a new harvesting behavior (this is similar to mutation rate, but agents can switch during their lifetimes)
* _chance agents move_: the likelihood that agents move around (up to one patch per iteration)
...
*_harvest-type_: agents change their harvest type based on imitation rules and behavior innovation rate
*_harvested amount_: the amount of resource units that is harvested by an agent in the current iteration
*_Energy level_: the energy level (in resource units) on an agent 
*_Age_: each agent has a certain age, in each iteration the agent gets older by 0.1 units
...
* The initial level of energy of agents is set at _living costs_ + 1.
* The _age_ of each agent is set randomly between 30 and 50.
...
![Model processes image](https://openevo.eva.mpg.de/wp-content/uploads/NetLogo-model-evolution-resource-use-behavior-imitation-procedures-overview.png)
...
**Trait variation**
There can be two variants of harvesting behavior in the population - sustainable or greedy. New trait variants can be introduced randomly in the population through the parameter _behavior innovation rate_.
...
**Trait inheritance and trait transmission**
Agents create offspring and inherit their harvesting behavior/type to their offspring. In case of other imitation biases, agents also “inherit” the behavior of other agents in their environment by imitating them.
...
**Cultural selection**
Traits are preferentially imitated based on the imitation bias, e.g. based on the success in terms of energy levels that the trait brings, or based on the frequency of the trait.

**Norms and traditions**
The imitation bias “majority” can establish norms of behavior in the population or in sub-groups of the population. Similarly, the imitation bias “imitate most experienced” can establish norms and traditions in the population or in sub-groups of the population. 

**Sensing and Information Processing** 
* Agents sense their environment in a radius of 2 patches: if a patch is occupied, and the patch with the most resources; 
* Agents sense all the other agents within their neighborhood of 8 patches: their harvesting behavior, their energy levels, and their age.
* Agents do not store any information (i.e. have no memory).


**Objectives and goal-directed behavior**
* Agents move to an unoccupied patch with the most resources in a radius of 2 patches (depending on _chance-agents-move_)
* Agents harvest their predetermined harvest amount (or until there are no more resources). 
* Agents imitate other agents based on the rule determined by _imitation-bias_.

**Learning and Adaptation**
Agents adapt their behavior over their lifetimes through the imitation bias.
...
* Agents interact directly through imitating each other.
...
* Agents move with a probability of _chance-agents-move_.
* Agents move to a randomly selected patch if several patches fulfill the objectives.
* Agents imitate with a probability of _imitation-likelihood_.
* Agents switch to a different behavior with a probability of _behavior-innovation-rate_.
...
Teaching materials for this model: https://openevo.eva.mpg.de/teachingbase/evolution-of-resource-use-through-behavior-imitation/
...
* Hanisch, S. (2022). Evolution of resource use through behavior imitation. OpenEvo NetLogo Models. https://openevo.eva.mpg.de/teachingbase/netlogo/
